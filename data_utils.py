...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_mel_text_pair(self, audiopath_and_text):
        ...
        mel = self.get_mel(os.path.join(self.hparams.wav_dir, audiopath))
        return (text, mel)
    ...
    def get_mel(self, filename):
        if not self.load_mel_from_disk:
            ...
            audio_norm = audio_norm.unsqueeze(0)## unsqueeze后变成(1,T)形状
            audio_norm = torch.autograd.Variable(audio_norm, requires_grad=False)

            ##传进去的是waveform形式的,返回的是（1，channel=80，386），这个386怎么来的我暂时不清楚
            melspec = self.stft.mel_spectrogram(audio_norm)
            # melspec=librosa.feature.melspectrogram(audio_norm,
